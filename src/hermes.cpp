 ...
 *
 * This sketch shows how to get the data from auto-calibration (offsets) and
 *reuse it. If you let line 20 commented, you will get the offsets. If you
 *uncomment line 20, you will apply the offsets.
 *
 * You can copy the offsets manually to reuse them, or you save them to / read
 *them from the EEPROM. For using the EEPROM uncomment line 21. If you use a non
 *AVR-based board you might need to adjust the code regarding the EEPROM use.
 *
...
/* Uncomment the following line after you have determined the offsets and copied
 * them */
...
#include <Wire.h>
constexpr unsigned MPU9250_ADDR = 0x68;
...
void setup() {
   ...
   * Uncomment "#define reuse_offsets" at the top if you want to reuse previous
   * offsets.
   */
  ...
  char buffer[35];
  sprintf(buffer, "{%d.0, %d.0, %d.0}", (int)aOffs.x, (int)aOffs.y,
          (int)aOffs.z);
  Serial.print("Acceleration offsets: ");
  Serial.println(buffer);
  sprintf(buffer, "{%d.0, %d.0, %d.0}", (int)gOffs.x, (int)gOffs.y,
          (int)gOffs.z);
  ...
  /* Alternatively read your offsets back from EEPROM, if you have saved them
   * there */
#ifdef USE_EEPROM
